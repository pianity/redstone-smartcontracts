...
class LexicographicalInteractionsSorter {
    constructor(arweave) {
        ...
        this.logger = _warp_1.LoggerFactory.INST.create('LexicographicalInteractionsSorter');
    }
    ...
    async addSortKey(txInfo) {
        ...
        if (txInfo.node.sortKey !== undefined && txInfo.node.source == _warp_1.SourceType.WARP_SEQUENCER) {
    ...
    async createSortKey(blockId, transactionId, blockHeight) {
        ...
        const hashed = (0, _warp_1.arrayToHex)(await this.arweave.crypto.hash(concatenated));

    ...
    async doReadState(missingInteractions, baseState, executionContext) {
        ...
        for (let i = 0; i < missingInteractionsLength; i++) {
            ...
            if (signal === null || signal === void 0 ? void 0 : signal.aborted) {
                throw new HandlerExecutorFactory_1.AbortError(`Abort signal in ${DefaultStateEvaluator.name}`);
            }
            const missingInteraction = missingInteractions[i];
            currentSortKey = missingInteraction.sortKey;
            contract
                .interactionState()
                .setInitial(contract.txId(), new StateEvaluator_1.EvalStateResult(currentState, validity, errorMessages), currentSortKey);
            ...
            if (isInteractWrite && internalWrites) {
                ...
                try {
                    ...
                    if ((e.name == JsHandlerApi_1.KnownErrors.ContractError &&
                        (e.subtype == 'unsafeClientSkip' || e.subtype == 'constructor' || e.subtype == 'blacklistedSkip')) ||
                        e.name == JsHandlerApi_1.KnownErrors.NonWhitelistedSourceError) {
                ...
                if (event) {
                    warp.eventTarget.dispatchEvent(new StateEvaluator_1.CustomEvent(isValidInteraction ? 'interactionCompleted' : 'interactionFailed', { detail: event }));
                }
            }
            if (progressPlugin) {
            ...
            try {
                ...
                if ((e.name == JsHandlerApi_1.KnownErrors.ContractError && e.subtype == 'unsafeClientSkip') ||
                    e.name == JsHandlerApi_1.KnownErrors.NonWhitelistedSourceError) {
                    validity[missingInteraction.id] = false;
            ...
            const forceStateStoreToCache = executionContext.evaluationOptions.cacheEveryNInteractions > 0 &&
            ...
            if (contract.isRoot()) {
                ...
                const interactionState = new StateEvaluator_1.EvalStateResult(currentState, validity, errorMessages);

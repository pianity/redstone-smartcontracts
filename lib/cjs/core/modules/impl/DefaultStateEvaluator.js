...
class DefaultStateEvaluator {
    constructor(arweave, executionContextModifiers = []) {
        ...
        this.logger = _warp_1.LoggerFactory.INST.create('DefaultStateEvaluator');
        this.tagsParser = new _warp_1.TagsParser();
    }
    async eval(executionContext, currentTx) {
        return this.doReadState(executionContext.sortedInteractions, new _warp_1.EvalStateResult(executionContext.contractDefinition.initState, {}), executionContext, currentTx);
    }
    async doReadState(missingInteractions, baseState, executionContext, currentTx) {
        ...
        for (let i = 0; i < missingInteractionsLength; i++) {
            ...
            const singleInteractionBenchmark = _warp_1.Benchmark.measure();
            ...
            if (isInteractWrite && internalWrites) {
                ...
                const writingContract = executionContext.warp.contract(writingContractTxId, executionContext.contract, interactionTx);
                ...
                if (newState !== null) {
                    ...
                    const toCache = new _warp_1.EvalStateResult(currentState, validity);
                    ...
                    if ((0, _warp_1.canBeCached)(interactionTx)) {
                ...
                const result = await executionContext.handler.handle(executionContext, new _warp_1.EvalStateResult(currentState, validity), interactionData);
        ...
        const evalStateResult = new _warp_1.EvalStateResult(currentState, validity);

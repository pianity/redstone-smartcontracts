...
class WarpGatewayInteractionsLoader {
    constructor(baseUrl, confirmationStatus = null, source = null) {
        ...
        this.source = source;
        this.logger = _warp_1.LoggerFactory.INST.create('WarpGatewayInteractionsLoader');
    ...
    async load(contractId, fromBlockHeight, toBlockHeight, evaluationOptions, upToTransactionId) {
        ...
        let page = 0;
        let totalPages = 0;
        const benchmarkTotalTime = _warp_1.Benchmark.measure();
        do {
            const benchmarkRequestTime = _warp_1.Benchmark.measure();
            ...
            const response = await fetch(`${url}?${new URLSearchParams({
                ...
                switch (error.status) {
                    case 504:
                        throw new _warp_1.InteractionsLoaderError('BadGatewayResponse504', errorMessage, error);
                    case 500:
                        throw new _warp_1.InteractionsLoaderError('BadGatewayResponse500', errorMessage, error);
                    default:
                        throw new _warp_1.InteractionsLoaderError('BadGatewayResponse', errorMessage, error);
                }
            });

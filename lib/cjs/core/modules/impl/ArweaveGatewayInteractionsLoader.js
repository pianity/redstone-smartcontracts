...
class ArweaveGatewayInteractionsLoader {
    constructor(arweave) {
        ...
        this.logger = _warp_1.LoggerFactory.INST.create('ArweaveGatewayInteractionsLoader');
    ...
    async load(contractId, fromBlockHeight, toBlockHeight, evaluationOptions) {
        ...
        const mainTransactionsVariables = {
            tags: [
                {
                    name: _warp_1.SmartWeaveTags.APP_NAME,
                    values: ['SmartWeaveAction']
                },
                {
                    name: _warp_1.SmartWeaveTags.CONTRACT_TX_ID,
                    values: [contractId]
                }
            ],
            blockFilter: {
        ...
        const loadingBenchmark = _warp_1.Benchmark.measure();
    ...
    async getNextPage(variables) {
        ...
        while (response.status === 403) {
            ...
            await (0, _warp_1.sleep)(ArweaveGatewayInteractionsLoader._30seconds);

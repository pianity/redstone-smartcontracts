...
class ContractHandlerApi {
    ...
    contractFunction, contractDefinition) {
        ...
        this.logger = _warp_1.LoggerFactory.INST.create('ContractHandlerApi');
        this.contractLogger = _warp_1.LoggerFactory.INST.create(swGlobal.contract.id);
    ...
    async handle(executionContext, currentResult, interactionData) {
        const { timeoutId, timeoutPromise } = (0, _warp_1.timeout)(executionContext.evaluationOptions.maxInteractionEvaluationTimeSeconds);
        try {
            const { interaction, interactionTx, currentTx } = interactionData;
            const stateCopy = (0, _warp_1.deepCopy)(currentResult.state, executionContext.evaluationOptions.useFastCopy);

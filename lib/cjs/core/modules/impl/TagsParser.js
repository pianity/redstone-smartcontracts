...
class TagsParser {
    constructor() {
        this.logger = _warp_1.LoggerFactory.INST.create('TagsParser');
    }
    getInputTag(interactionTransaction, contractTxId) {
        ...
        if (TagsParser.hasMultipleInteractions(interactionTransaction)) {
            ...
            const contractTagIndex = interactionTransaction.node.tags.findIndex((tag) => tag.name === _warp_1.SmartWeaveTags.CONTRACT_TX_ID && tag.value === contractTxId);
            ...
            if (inputTag.name !== _warp_1.SmartWeaveTags.INPUT) {
            ...
            return interactionTransaction.node.tags.find((tag) => tag.name === _warp_1.SmartWeaveTags.INPUT);
        }
    }
    isInteractWrite(interactionTransaction, contractTxId) {
        return interactionTransaction.node.tags.some((tag) => tag.name === _warp_1.SmartWeaveTags.INTERACT_WRITE && tag.value === contractTxId);
    }
    getInteractWritesContracts(interactionTransaction) {
        return interactionTransaction.node.tags
            .filter((tag) => tag.name === _warp_1.SmartWeaveTags.INTERACT_WRITE)
            .map((t) => t.value);
    }
    getContractTag(interactionTransaction) {
        var _a;
        return (_a = interactionTransaction.node.tags.find((tag) => tag.name === _warp_1.SmartWeaveTags.CONTRACT_TX_ID)) === null || _a === void 0 ? void 0 : _a.value;
    }
    getContractsWithInputs(interactionTransaction) {
        const result = new Map();
        const contractTags = interactionTransaction.node.tags.filter((tag) => tag.name === _warp_1.SmartWeaveTags.CONTRACT_TX_ID);
    ...
    static hasMultipleInteractions(interactionTransaction) {
        return interactionTransaction.node.tags.filter((tag) => tag.name === _warp_1.SmartWeaveTags.CONTRACT_TX_ID).length > 1;
    }
}

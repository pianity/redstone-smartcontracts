...
const InteractionsLoader_1 = require("../core/modules/InteractionsLoader");
/**
...
class HandlerBasedContract {
    ...
    async bundleInteraction(input, options) {
        ...
        if (!this.signer) {
            throw new Contract_1.BundleInteractionError({ type: 'NoWalletConnected' }, "Wallet not connected. Use 'connect' method first.");
        }
        let interactionTx;
        try {
            interactionTx = await this.createInteraction(input, options.tags, CreateContract_1.emptyTransfer, options.strict, true, options.vrf);
        }
        catch (e) {
            if (e instanceof InteractionsLoader_1.InteractionsLoaderError) {
                throw new Contract_1.BundleInteractionError(e.detail, `${e}`, e);
            }
            else {
                throw new Contract_1.BundleInteractionError({ type: 'InvalidInteraction' }, `${e}`, e);
            }
        }
        const response = await fetch(`${this._evaluationOptions.bundlerUrl}gateway/sequencer/register`, {
            ...
            throw new Contract_1.BundleInteractionError({ type: 'CannotBundle' }, `Unable to bundle interaction: ${JSON.stringify(error)}`);
        });
        return {

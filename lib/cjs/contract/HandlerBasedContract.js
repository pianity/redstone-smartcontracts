...
class HandlerBasedContract {
    ...
    async bundleInteraction(input, options = {
        ...
        let interactionTx;
        try {
            interactionTx = await this.createInteraction(input, options.tags, _smartweave_1.emptyTransfer, options.strict, true, options.vrf);
        }
        catch (e) {
            if (e instanceof _smartweave_1.InteractionsLoaderError) {
                throw new _smartweave_1.BundleInteractionError('BadGatewayResponse', `${e}`, e);
            }
            else {
                throw new _smartweave_1.BundleInteractionError('InvalidInteraction', `${e}`, e);
            }
        }
    ...
    async createInteraction(input, tags, transfer, strict, bundle = false, vrf = false) {
        ...
        if (vrf) {
            tags.push({
                name: _smartweave_1.SmartWeaveTags.REQUEST_VRF,
                value: 'true'
            });
        }
        const interactionTx = await (0, _smartweave_1.createTx)(this.smartweave.arweave, this.signer, this._contractTxId, input, tags, transfer.target, transfer.winstonQty, bundle);

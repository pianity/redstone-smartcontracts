...
class HandlerBasedContract {
    ...
    async readState(sortKeyOrBlockHeight, interactions, signal) {
        var _a, _b, _c;
        ...
        try {
            ...
            const executionContext = await this.createExecutionContext(this._contractTxId, sortKey, false, interactions, signal);
    ...
    async readStateFor(sortKey, interactions, signal) {
    ...
    async viewState(input, tags = [], transfer = CreateContract_1.emptyTransfer, caller, signal) {
        this.logger.info('View state for', this._contractTxId);
        return await this.callContract(input, 'view', caller, undefined, tags, transfer, false, false, true, signal);
    }
    ...
    async createExecutionContext(contractTxId, upToSortKey, forceDefinitionLoad = false, interactions, signal) {
        var _a, _b, _c, _d, _e;
        ...
        cachedState = cachedState || (await stateEvaluator.latestAvailableState(contractTxId, upToSortKey));

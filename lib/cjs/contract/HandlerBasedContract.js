...
class HandlerBasedContract {
    ...
    async writeInteraction(input, tags = [], transfer = _warp_1.emptyTransfer, strict = false) {
        ...
        return (0, neverthrow_1.ok)(interactionTx.value.id);
    }
    async bundleInteraction(input, options = {
        ...
        const interactionTx = await this.createInteraction(input, options.tags, _warp_1.emptyTransfer, options.strict, true, options.vrf);
        if (interactionTx.isErr()) {
            return (0, neverthrow_1.err)(interactionTx.error);
        }
    ...
    async createInteraction(input, tags, transfer, strict, bundle = false, vrf = false) {
        if (this._evaluationOptions.internalWrites) {
            ...
            if (strict) {
                ...
                if (handlerResult.type !== 'ok') {
                    return (0, neverthrow_1.err)({ error: 'invalidInteraction', message: handlerResult.errorMessage });
                }
            }
        }
        if (vrf) {
        ...
        return (0, neverthrow_1.ok)(interactionTx);
    }
    txId() {

...
class HandlerBasedContract {
    constructor(_contractTxId, warp, _parentContract = null, _callingInteraction = null) {
        ...
        this.logger = _warp_1.LoggerFactory.INST.create('HandlerBasedContract');
        this._evaluationOptions = new _warp_1.DefaultEvaluationOptions();
        /**
        ...
        this._innerWritesEvaluator = new _warp_1.InnerWritesEvaluator();
        this._benchmarkStats = null;
        this.waitForConfirmation = this.waitForConfirmation.bind(this);
        this._arweaveWrapper = new _warp_1.ArweaveWrapper(warp.arweave);
        if (_parentContract != null) {
            ...
            const callStack = new _warp_1.ContractCallStack(_contractTxId, this._callDepth);
            interaction.interactionInput.foreignContractCalls.set(_contractTxId, callStack);
            this._callStack = callStack;
        }
        else {
            this._callDepth = 0;
            this._callStack = new _warp_1.ContractCallStack(_contractTxId, 0);
        }
    }
    ...
    async readStateSequencer(blockHeight, upToTransactionId, currentTx) {
        ...
        const initBenchmark = _warp_1.Benchmark.measure();
        ...
        initBenchmark.stop();
        const stateBenchmark = _warp_1.Benchmark.measure();
    ...
    async viewState(input, blockHeight, tags = [], transfer = _warp_1.emptyTransfer) {
    ...
    async writeInteraction(input, tags = [], transfer = _warp_1.emptyTransfer, strict = false) {
    ...
    async bundleInteraction(input, options = {
        ...
        if (!this.signer) {
            throw new _warp_1.BundleInteractionError({ type: 'NoWalletConnected' }, "Wallet not connected. Use 'connect' method first.");
        }
        options = {
            tags: [],
            strict: false,
        ...
        let interactionTx;
        try {
            interactionTx = await this.createInteraction(input, options.tags, _warp_1.emptyTransfer, options.strict, true, options.vrf);
        }
        catch (e) {
            if (e instanceof _warp_1.InteractionsLoaderError) {
                throw new _warp_1.BundleInteractionError(e.detail, `${e}`, e);
            }
            else {
                throw new _warp_1.BundleInteractionError({ type: 'InvalidInteraction' }, `${e}`, e);
            }
        }
        const response = await fetch(`${this._evaluationOptions.bundlerUrl}gateway/sequencer/register`, {
            ...
            throw new _warp_1.BundleInteractionError({ type: 'CannotBundle' }, `Unable to bundle interaction: ${JSON.stringify(error)}`);
        });
        return {
    ...
    async createInteraction(input, tags, transfer, strict, bundle = false, vrf = false) {
        if (this._evaluationOptions.internalWrites) {
            ...
            innerWrites.forEach((contractTxId) => {
                tags.push({
                    name: _warp_1.SmartWeaveTags.INTERACT_WRITE,
                    value: contractTxId
                });
            });
        ...
        if (vrf) {
            tags.push({
                name: _warp_1.SmartWeaveTags.REQUEST_VRF,
                value: 'true'
            });
        }
        const interactionTx = await (0, _warp_1.createTx)(this.warp.arweave, this.signer, this._contractTxId, input, tags, transfer.target, transfer.winstonQty, bundle);
    ...
    async createExecutionContext(contractTxId, blockHeight, forceDefinitionLoad = false, upToTransactionId = undefined) {
        var _a;
        const { definitionLoader, interactionsLoader, interactionsSorter, executorFactory, stateEvaluator, useWarpGwInfo } = this.warp;
        ...
        benchmark.reset();
        const evolvedSrcTxId = _warp_1.Evolve.evolvedSrcTxId((_a = cachedState === null || cachedState === void 0 ? void 0 : cachedState.cachedValue) === null || _a === void 0 ? void 0 : _a.state);
        ...
        const containsInteractionsFromSequencer = interactions.some((i) => i.node.source == _warp_1.SourceType.WARP_SEQUENCER);
    ...
    maybeResetRootContract(blockHeight) {
        if (this._parentContract == null) {
            ...
            this._networkInfo = null;
            this._callStack = new _warp_1.ContractCallStack(this.txId(), 0);
            this._rootBlockHeight = blockHeight;
        }
    }
    async callContract(input, caller, blockHeight, tags = [], transfer = _warp_1.emptyTransfer) {
        var _a;
        ...
        const tx = await (0, _warp_1.createTx)(arweave, this.signer, this._contractTxId, input, tags, transfer.target, transfer.winstonQty);
        const dummyTx = (0, _warp_1.createDummyTx)(tx, executionContext.caller, executionContext.currentBlockData);
    ...
    async evolve(newSrcTxId, useBundler = false) {
        if (useBundler) {
            return await this.bundleInteraction({ function: 'evolve', value: newSrcTxId });
        }
        else {
    ...
    async save(sourceData) {
        ...
        const { arweave } = this.warp;
        const source = new _warp_1.SourceImpl(arweave);

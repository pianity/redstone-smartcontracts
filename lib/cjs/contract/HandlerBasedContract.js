...
class HandlerBasedContract {
    ...
    async writeInteraction(input, options) {
        ...
        if (bundleInteraction) {
            ...
            const interaction = await this.createInteraction(input, effectiveTags, effectiveTransfer, effectiveStrict, false, effectiveVrf && environment !== 'mainnet', effectiveReward);
    ...
    async bundleInteraction(input, options) {
        ...
        const interaction = await this.createInteraction(input, options.tags, CreateContract_1.emptyTransfer, options.strict, true, options.vrf);
        if ((options.strict && interaction.type == 'ok') || !options.strict) {
    ...
    async createInteraction(input, tags, transfer, strict, bundle = false, vrf = false, reward) {
        ...
        if (this._evaluationOptions.internalWrites) {
            ...
            const innerHandlerResult = await this.callContract(input, undefined, undefined, tags, transfer, strict, vrf);
            if (strict) {
            ...
            handlerResult = await this.callContract(input, undefined, undefined, tags, transfer, strict, vrf);
        }
        let interactionTx;
        if ((strict && (handlerResult === null || handlerResult === void 0 ? void 0 : handlerResult.type) === 'ok') || !strict) {
            if (vrf) {
                tags.push({
            ...
            interactionTx = await (0, create_interaction_tx_1.createInteractionTx)(this.warp.arweave, this.signer, this._contractTxId, input, tags, transfer.target, transfer.winstonQty, bundle, reward);
        }
        if (strict) {
            const resultStrict = {
                type: (_a = handlerResult === null || handlerResult === void 0 ? void 0 : handlerResult.type) !== null && _a !== void 0 ? _a : 'ok',
                interactionTx
            };
            if ((handlerResult === null || handlerResult === void 0 ? void 0 : handlerResult.type) === 'error' && (handlerResult === null || handlerResult === void 0 ? void 0 : handlerResult.error)) {

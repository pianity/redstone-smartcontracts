...
class HandlerBasedContract {
    ...
    async bundleInteraction(input, options = {
        ...
        if (!this.signer) {
            throw new _warp_1.BundleInteractionError('NoWalletConnected', "Wallet not connected. Use 'connect' method first.");
        }
        options = {
            tags: [],
            strict: false,
        ...
        try {
            ...
            if (e instanceof _warp_1.InteractionsLoaderError) {
                throw new _warp_1.BundleInteractionError('BadGatewayResponse', `${e}`, e);
            }
            else {
                throw new _warp_1.BundleInteractionError('InvalidInteraction', `${e}`, e);
            }
        }
        const response = await fetch(`${this._evaluationOptions.bundlerUrl}gateway/sequencer/register`, {
            ...
            throw new _warp_1.BundleInteractionError('CannotBundle', `Unable to bundle interaction: ${JSON.stringify(error)}`);
        });
        return {

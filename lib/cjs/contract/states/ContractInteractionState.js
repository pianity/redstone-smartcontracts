...
class ContractInteractionState {
    ...
    async getKV(contractTxId, cacheKey) {
        ...
        return ((_a = (await storage.get(cacheKey))) === null || _a === void 0 ? void 0 : _a.cachedValue) || null;
    }
    async delKV(contractTxId, cacheKey) {
        const storage = await this.getOrInitKvStorage(contractTxId);
        await storage.del(cacheKey);
    }
    async getKvKeys(contractTxId, sortKey, options) {
        const storage = await this.getOrInitKvStorage(contractTxId);
        return storage.keys(sortKey, options);
    }
    async getKvRange(contractTxId, sortKey, options) {
        const storage = await this.getOrInitKvStorage(contractTxId);

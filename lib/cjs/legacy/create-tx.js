...
async function createTx(arweave, signer, contractId, input, tags, target = '', winstonQty = '0', bundle = false) {
    ...
    interactionTx.addTag(_warp_1.SmartWeaveTags.APP_NAME, 'SmartWeaveAction');
    // use real SDK version here?
    interactionTx.addTag(_warp_1.SmartWeaveTags.APP_VERSION, '0.3.0');
    interactionTx.addTag(_warp_1.SmartWeaveTags.SDK, 'Warp');
    interactionTx.addTag(_warp_1.SmartWeaveTags.CONTRACT_TX_ID, contractId);
    interactionTx.addTag(_warp_1.SmartWeaveTags.INPUT, JSON.stringify(input));
    if (signer) {

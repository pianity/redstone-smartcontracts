...
export function configureSandbox(sandbox, arweave, swGlobal) {
    // ContractAssert
    ...
    sandbox.setSync('__host__smartweave__contracts_readContractState', new ivm.Reference(async function (...args) {
        // eslint-disable-next-line prefer-spread
        const result = await swGlobal.contracts.readContractState.apply(swGlobal, args);
        return new ivm.ExternalCopy(result);
    }));
    sandbox.setSync('__host__smartweave__contracts_write', new ivm.Reference(async function (...args) {
        const result = await swGlobal.contracts.write.apply(swGlobal, args);
        return new ivm.ExternalCopy(result);
    }));
    sandbox.setSync('__host__smartweave__contracts_viewContractState', new ivm.Reference(async function (...args) {
        const result = await swGlobal.contracts.viewContractState.apply(swGlobal, args);
        return new ivm.ExternalCopy(result);
    }));
    sandbox.setSync('__host__smartweave__contracts_refreshState', new ivm.Reference(async function (...args) {
        const result = await swGlobal.contracts.refreshState.apply(swGlobal, args);
        return new ivm.ExternalCopy(result);
    }));
    // SmartWeave - getBalance
    sandbox.setSync('__host__smartweave_getBalance', new ivm.Reference(async function (...args) {
        // eslint-disable-next-line prefer-spread
    ...
    sandbox.setSync('__host__smartweave__arweave__wallets_getBalance', new ivm.Reference(async function () {
        throw new Error('this function is non-deterministic');
    }));
    sandbox.setSync('__host__smartweave__arweave__wallets_getLastTransactionID', new ivm.Reference(async function () {
        throw new Error('this function is non-deterministic');
    }));
    sandbox.setSync('__host__smartweave__arweave__wallets_generate', new ivm.Reference(async function () {
        throw new Error('this function is non-deterministic');
    }));
    ...
    sandbox.setSync('__host__smartweave__arweave__wallets_ownerToAddress', new ivm.Reference(arweave.wallets.ownerToAddress));
    ...
    sandbox.setSync('__host__smartweave__arweave__crypto_generateJWK', new ivm.Reference(async function () {
        throw new Error('this function is non-deterministic');
    }));
    sandbox.setSync('__host__smartweave__arweave__crypto_sign', new ivm.Reference(async function () {
        throw new Error('this function is non-deterministic');
    }));
    ...
    sandbox.setSync('__host__smartweave__arweave__crypto_hash', new ivm.Reference(async function (...args) {
        // eslint-disable-next-line prefer-spread
        const result = await arweave.crypto.hash.apply(arweave.crypto, args);
        return new ivm.ExternalCopy(result);
    }));
    // arweave - unsafeClient - transactions
    sandbox.setSync('__host__smartweave__arweave__transactions_get', new ivm.Reference(async function (...args) {
        // eslint-disable-next-line prefer-spread
        const result = await arweave.transactions.get.apply(arweave.transactions, args);
        return new ivm.ExternalCopy(result);
    }));
    sandbox.setSync('__host__smartweave__arweave__transactions_getData', new ivm.Reference(async function (...args) {
        // eslint-disable-next-line prefer-spread

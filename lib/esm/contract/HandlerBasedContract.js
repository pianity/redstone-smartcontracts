import { ArweaveWrapper, Benchmark, BundleInteractionError, ContractCallStack, createDummyTx, createTx, DefaultEvaluationOptions, emptyTransfer, Evolve, InnerWritesEvaluator, InteractionsLoaderError, LoggerFactory, sleep, SmartWeaveTags, SourceType, SourceImpl } from '..';
...
export class HandlerBasedContract {
    ...
    async bundleInteraction(input, options = {
        ...
        if (!this.signer) {
            throw new BundleInteractionError({ type: 'NoWalletConnected' }, "Wallet not connected. Use 'connect' method first.");
        }
        options = {
            tags: [],
            strict: false,
        ...
        let interactionTx;
        try {
            interactionTx = await this.createInteraction(input, options.tags, emptyTransfer, options.strict, true, options.vrf);
        }
        catch (e) {
        ...
        const response = await fetch(`${this._evaluationOptions.bundlerUrl}gateway/sequencer/register`, {
            ...
            throw new BundleInteractionError({ type: 'CannotBundle' }, `Unable to bundle interaction: ${JSON.stringify(error)}`);
        });
        return {
    ...
    async createInteraction(input, tags, transfer, strict, bundle = false, vrf = false) {
        ...
        const interactionTx = await createTx(this.warp.arweave, this.signer, this._contractTxId, input, tags, transfer.target, transfer.winstonQty, bundle);
    ...
    async callContract(input, caller, blockHeight, tags = [], transfer = emptyTransfer) {
        ...
        const tx = await createTx(arweave, this.signer, this._contractTxId, input, tags, transfer.target, transfer.winstonQty);

import { ArweaveWrapper, Benchmark, ContractCallStack, createDummyTx, createTx, DefaultEvaluationOptions, emptyTransfer, Evolve, InnerWritesEvaluator, LoggerFactory, sleep, SmartWeaveTags, SourceType, SourceImpl } from '..';
...
export class HandlerBasedContract {
    ...
    async bundleInteraction(input, options = {
        ...
        const interactionTx = await this.createInteraction(input, options.tags, emptyTransfer, options.strict, true, options.vrf);
        if (interactionTx.isErr()) {
            return err(interactionTx.error);
        }
